...
public class ConceptSMTKWeb extends ConceptSMTK implements Serializable {
    ...
    *  @return Una <code>java.util.List</code> de relaciones removidas
    ...
    public void autogenerate() {
        ...
        for (RelationshipDefinition relationshipDefinition : getCategory().getRelationshipDefinitions()) {
            if(relationshipDefinition.isAutogenerable()) {
                List<RelationshipWeb> autogenerableRelationships = getValidRelationshipsWebByRelationDefinition(relationshipDefinition);
                Collections.sort(autogenerableRelationships);
                for (Relationship relationship : autogenerableRelationships) {
                    autogeneratedTerm = autogeneratedTerm + " + " + relationship.getTarget().toString();
                }    
            }
        }
        getValidDescriptionFavorite().setTerm(autogeneratedTerm);
        getValidDescriptionFSN().setTerm(autogeneratedTerm);
    }
