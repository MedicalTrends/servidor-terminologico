...
import static cl.minsal.semantikos.model.LoadLog.*;

/**
 * Created by root on 15-06-17.
 */
public class PCConceptLoaderFix extends EntityLoader {
    ...
    private static final Logger logger = Logger.getLogger(PCConceptLoaderFix.class.getName() );
    ...
    public void loadConceptFromFileLine(String line, User user) throws LoadException, ParseException {
        ...
        try {
            ...
            //Eliminar el regnum/regaño mas antiguo
            for (Relationship rel : concept.getRelationshipsByRelationDefinition(category.findRelationshipDefinitionsByName(TargetDefinition.ISP).get(0))) {
                ...
                if(regnumBD.equals(regNum)) {
                    if(!reganoBD.equals(regAno)) {
                        concept.getRelationships().remove(rel);
                        conceptSMTKMap.put(conceptID, new Pair<>(_concept, concept));
                        log(new LoadException(path.toString(), conceptID, "Concepto '" + concept.toString() + " se elimina relación '" + rel + "'", INFO, "N"));
                        break;
                    }
                }
            }
    ...
    public void loadAllConcepts(SMTKLoader smtkLoader) {
        ...
        try {
            ...
            initWriter("Fármacos_-_Producto_Comercial_fix");
